...
<div style="max-width:auto; margin:auto;">
  ...
  <head>
    ...
    <style>
      ...
      label {
        display: flex; /* block on its own line */
        align-items: center;
        margin-bottom: 6px;
        gap: 8px;
        cursor: pointer;
  ...
  <body>
    ...
    <div id="perItemInputs" style="display: none; margin-left: 20px;">
      <label>How many techniques per set?
        <input type="number" id="perItemCount" min="1" value="1" style="width: 60px;">
      </label>
      <label>
    ...
    <div id="singleCountInput" style="margin-top: 10px;">
      <label for="numberToGenerate">How many techniques?</label>
      <input type="number" id="numberToGenerate" min="1" value="1" style="width: 60px;">
    </div>
    ...
    <div id="feedback-buttons" style="text-align: center; display: none; margin-top: 20px;">
    ...
    <script>
      ...
      // When a category radio changes, update the checkboxes to match that category's full set
      document.querySelectorAll('input.category').forEach(radio => {
        radio.addEventListener('change', () => {
          if (radio.checked) {
            const cat = radio.dataset.category;
            // Uncheck all checkboxes
            document.querySelectorAll('input.item').forEach(cb => cb.checked = false);

            // Get all items from the category and subcategories recursively
            let itemsToCheck = gatherCategoryItems(cat);

            // Check those checkboxes that match itemsToCheck
            document.querySelectorAll('input.item').forEach(cb => {
              if (itemsToCheck.includes(cb.value)) {
                cb.checked = true;
              }
            });
          }
      ...
      // If any checkbox changes, clear the category radios (to prevent confusion)
      document.querySelectorAll('input.item').forEach(cb => {
        cb.addEventListener('change', () => {
          if (cb.checked) {
            document.querySelectorAll('input.category').forEach(radio => radio.checked = false);
          }
        });
      });
      ...
      function startGeneration() {
        ...
        // First check if a category radio is selected
        ...
        if (checkedCategory) {
          ...
          // No category radio, so get checked checkboxes
          const checkedBoxes = document.querySelectorAll('input.item:checked');
          checkedBoxes.forEach(cb => selectedItems.push(cb.value));
        }
        ...
        if (isNaN(perItemCount) || perItemCount < 1) {
          alert('Please enter a valid number of techniques.');
          return;
        }
      ...
      function buildList(items, count) {
        let list = [];
        items.forEach(item => {
          // Find limit by checking the corresponding checkbox's data-limit attribute
          let limit = 10;
          const checkbox = Array.from(document.querySelectorAll('input.item')).find(cb => cb.value === item);
          if (checkbox && checkbox.dataset.limit) {
            limit = parseInt(checkbox.dataset.limit);
          }
